...
pub extern "C" fn _start(pointer: *const BootstrapManifest, length: usize) -> ! {
    ...
    // A forged handle must not bind or control a real InterruptObject.
    let forged = Handle::from_parts(0xffff, 1);
    if syscall::interrupt_control_with_arg(irq, InterruptOp::BindNotification, forged.raw()).is_ok()
        || syscall::interrupt_control(forged, InterruptOp::Mask).is_ok()
    {
    ...
    let _ = syscall::debug_console_write(console, "GAXERA: IRQ_UNAUTHORIZED_REJECTED\n");
    syscall::exit(0);
}
