...
pub extern "C" fn _start(pointer: *const BootstrapManifest, length: usize) -> ! {
    ...
    if process_support::create_terminate_reap(factory, 0x54).is_err()
        || process_support::create_terminate_reap(factory, 0x55).is_err()
    {
    ...
    let _ = syscall::debug_console_write(console, "GAXERA: PROCESS_CREATE_REAP_REPEAT_SUCCESS\n");
    syscall::exit(0);
}
