    ...
    )))]
    {
        ...
        *kernel::global::PHYSICAL_ALLOCATOR.lock() = Some(physical_frames);
        ...
        #[cfg(feature = "test-ipc-priority-inheritance")]
        {
            ...
            // Server takes received call to get the exact reply token generated by make_reply_token
            let received_call = ep.take_received_call().unwrap();
            let reply_token = received_call.reply_token;

            // 4. Server replies and restores base priority
            assert!(ep.reply(reply_token, msg).is_ok());
